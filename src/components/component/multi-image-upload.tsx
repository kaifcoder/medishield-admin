...
type InputProps = {
  ...
  onFileRemove?: (file: FileState) => void | Promise<void>;
};
...
const MultiImageDropzone: any = React.forwardRef<HTMLInputElement, InputProps>(
  (
    {
      dropzoneOptions,
      value,
        ...
        <div className="grid grid-cols-[repeat(1,1fr)] gap-2 sm:grid-cols-[repeat(2,1fr)] lg:grid-cols-[repeat(3,1fr)] xl:grid-cols-[repeat(4,1fr)]">
          ...
          {value?.map(({ file, progress }, index) => (
            ...
            >
              ...
              />
              {/* remove button to remove from edgestore also*/}
              <div
                className="group absolute right-0 top-0 -translate-y-1/4 translate-x-1/4 transform"
                onClick={(e) => {
                  e.stopPropagation();
                  void onChange?.(value.filter((_, i) => i !== index) ?? []);
                  void onFileRemove?.(value[index]);
                }}
              >
